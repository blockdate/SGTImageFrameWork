...
- (BOOL)isZMJThisYear {
    ...
    // 当前年份
    NSInteger nowYear= [calender component:NSCalendarUnitYear fromDate:nowDate];
    // 传入日期的年份
    NSInteger thisYear = [calender component:NSCalendarUnitYear fromDate:nowDate];
    return (nowYear - thisYear) == 0;
}
/**
 *  是否是今天
 */
- (BOOL)isZMJToday {
    ...
    NSDateComponents *comps = [calender components:unit fromDate:self toDate:nowDate options:0];
...
- (BOOL)isZMJYesterday {
    ...
    NSCalendarUnit unit = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay;
    NSDateComponents *comps = [calender components:unit fromDate:self toDate:nowDate options:0];
    return !comps.year && !comps.month && comps.day == 1;
}
