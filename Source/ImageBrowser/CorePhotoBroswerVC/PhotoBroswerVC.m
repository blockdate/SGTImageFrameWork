...
@property (nonatomic, copy) NSDictionary* (^reportblock)(NSInteger);

@end

@implementation PhotoBroswerVC

+ (void)show:(UIViewController *)handleVC isLocal:(BOOL)islocal type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock tapedReportAtIndex:(NSDictionary* (^)(NSInteger))reportblock {
    //取出相册数组
...
+ (void)show:(UIViewController *)handleVC isLocal:(BOOL)islocal type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock {
    [self show:handleVC isLocal:islocal type:type index:index photoModelBlock:photoModelBlock tapedReportAtIndex:nil];
...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock{
    [self show:handleVC isLocal:YES type:type index:index photoModelBlock:photoModelBlock];
}
...
//    NSLog(@"%p",&photoItemView);
    ...
    photoItemView.ItemViewLongPressBlock = ^(){
        [self showActionSheetWithImageIndex:page];
    };
...
- (void)showActionSheetWithImageIndex:(NSInteger)index {
    if (_shouldLongPress) {
...
- (IBAction)rightBtnClick:(id)sender {
    ...
    if([itemModel read]){//已经保存过本地
        ...
        [itemView save:^{
            ...
            NSString *tipStr = @"保存失败";
            ALAuthorizationStatus author = [ALAssetsLibrary authorizationStatus];
            switch (author) {
                case ALAuthorizationStatusNotDetermined: //用户未对这个应用程序的权限做出选择
                    tipStr = @"保存失败:\n您还未授权应用访问相册权限.请授权后再进行保存!";
                    break;
                case ALAuthorizationStatusRestricted: //此应用程序没有被授权访问的照片数据。可能是家长控制权限
                    tipStr = @"保存失败:\n应用程序没有被授权访问的照片数据.可能是家长控制权限.";
                    break;
                case ALAuthorizationStatusDenied: //用户已经明确拒绝了此应用程序访问照片数据.
                    tipStr = @"保存失败:\n您已主动限制了应用访问相册的权限.请前往系统设置界面重新设置!";
                    break;
                case ALAuthorizationStatusAuthorized: //用户已授权此应用访问照片数据.
                    tipStr = @"保存失败:\n手机内存不足!";
                    break;
                default:
                    break;
            }
            [CoreSVP showSVPWithType:CoreSVPTypeError Msg:tipStr duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
...
- (UIActionSheet *)actionSheet {
    if (nil == _actionSheet) {
        _actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"举报" otherButtonTitles:@"保存", nil];
    }
    return _actionSheet;
}
