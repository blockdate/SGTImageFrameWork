...
@interface SGTPhotoPickerBrowser () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
{
...
@implementation SGTPhotoPickerBrowser

- (void)dealloc
{
    NSLog(@"%s DNPhotoBrowser",__FUNCTION__);
}
...
- (void)updateNavigationBarAndToolBar {
    ...
    if (self.isFullImage) {
        SGTImageAsset *dasset = self.photoDataSources[self.currentIndex];
        ALAsset *asset = dasset.alAsset;
        if (asset != nil) {
...
- (UICollectionView *)browserCollectionView
{
    if (nil == _browserCollectionView) {
        ...
        [_browserCollectionView registerClass:[SGTBrowserCell class] forCellWithReuseIdentifier:NSStringFromClass([SGTBrowserCell class])];
...
{
    SGTBrowserCell *cell = (SGTBrowserCell *)[collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([SGTBrowserCell class]) forIndexPath:indexPath];
