...
- (instancetype)initWithGroupURL:(NSURL *)assetsGroupURL
{
    ...
    if (self) {
        ...
        if ([DNAsset sharedLibrary] != nil) {
            _assetsLibrary = [DNAsset sharedLibrary];
        }else {
...
- (void)loadData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [self.assetsGroup enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            if (result) {
                [self.assetsArray insertObject:[[DNAsset alloc] initWithAlasset:result] atIndex:0];
            }
        }];
...
{
    DNAsset *asset = [[DNAsset alloc] initWithAlasset:ALAsset];
...
- (void)sendImages
{
    ...
    [DNAsset setSharedLibrary:_assetsLibrary];
